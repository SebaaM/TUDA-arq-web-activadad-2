...
function resolveErrorMessage(error: ApiError): string {
	// Mantiene los mensajes orientados a la persona separados de los códigos
	// técnicos que usa el cliente HTTP para clasificar cada fallo.
	switch (error.code) {
		case "bad_request":
		...
		default:
			return (
				error.message ||
				`Error inesperado${error.status !== null ? ` (HTTP ${error.status})` : ""}.`
			);
	}
}

function canRetry(error: ApiError | null): boolean {
	// Reintentar solo tiene sentido ante fallos potencialmente transitorios;
	// una identidad inválida o un recurso inexistente requiere otra acción.
	return (
		error?.code === "network" ||
...
export default function EnrollmentPanel({ activityId }: Props) {
	// Esta información vive en React porque cambia después de cargar el HTML
	// estático. `phase` y `busy` separan el estado visible de la acción activa.
	...
	// Se ejecuta solo en el navegador: localStorage no existe durante el SSR ni
	// durante el build de Astro. La variable pública sirve como valor inicial.
	useEffect(() => {
	...
	const queryEnrollment = useCallback(
		async (identity: string) => {
			// La consulta se centraliza para reutilizarla al hidratar y al reintentar.
			if (!identity) return;
			setBusy("query");
			setError(null);
			setNotice(null);
	...
	useEffect(() => {
		// El cambio de identidad dispara una nueva consulta y evita mostrar datos
		// del participante anterior.
		if (participantId !== null) void queryEnrollment(participantId);
	}, [participantId, queryEnrollment]);

	function handleSaveIdentity() {
		// Validar antes de persistir evita enviar headers imposibles y deja la
		// identidad anterior intacta si el UUID ingresado no es válido.
	...
	async function handleEnroll() {
		// El botón usa PUT: la API puede responder 201 si crea la inscripción o
		// 200 si ya existía, ambos casos dejan la isla en estado `enrolled`.
	...
	async function handleCancel() {
		// DELETE devuelve 204 sin cuerpo; al confirmar éxito se limpia el estado
		// local para que la interfaz refleje inmediatamente la cancelación.
		if (!participantId) return;
		setBusy("cancel");
		setError(null);
		setNotice(null);
